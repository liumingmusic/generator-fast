$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'business/tbgoods/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.tbGoods.goodsId == null ? "business/tbgoods/save" : "business/tbgoods/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.tbGoods),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "business/tbgoods/delete",
                    contentType: "application/json",
		...
		getInfo: function(goodsId){
			$.get(baseURL + "business/tbgoods/info/"+goodsId, function(r){
                vm.tbGoods = r.tbGoods;
            });
		},
